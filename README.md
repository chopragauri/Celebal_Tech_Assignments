# Celebal Tech Assignments

This repository contains my weekly assignments for the **Celebal Technologies Data Science Program**.

> 📌 **Note:** Earlier weeks were submitted through separate repositories. Links to all submissions are listed below week-wise.

---

## 📅 Week-wise Submissions

| Week | Topic | Repository |
|------|-------|------------|
| Week 1 | Getting Started / Foundations | [chopragauri/Celebal_Technologies](https://github.com/chopragauri/Celebal_Technologies) |
| Week 2 | Tesla ML Pipeline | [chopragauri/tesla-ml-pipeline](https://github.com/chopragauri/tesla-ml-pipeline) |
| Week 3 | ML Classification & Clustering Pipeline | [chopragauri/ml-classification-clustering-pipeline](https://github.com/chopragauri/ml-classification-clustering-pipeline) |
| Week 4 | CIFAR-10 ANN & CNN | [chopragauri/cifar10-ann-cnn](https://github.com/chopragauri/cifar10-ann-cnn) |
| Week 5 | Text Generation using RNN, LSTM & GRU | [Text_Generation_RNN_LSTM_GRU_Learning_Project_Solution.ipynb](./Text_Generation_RNN_LSTM_GRU_Learning_Project_Solution.ipynb) *(this repo)* |
| Week 6 | MNIST Denoising Autoencoder | [week6_gauriChopra.ipynb](./week6_gauriChopra.ipynb) *(this repo)* |
| Week 7 | Document Q&A System (RAG) | [week7_RAG/week7_gauriChopra.ipynb](./week7_RAG/week7_gauriChopra.ipynb) *(this repo)* |

---

*From Week 5 onwards, all submissions are consolidated in this repository.*
